...
- (void)testAdd;{
    ...
    XCTAssert([provider isAddress:primeriAddress containsTx:tx0]);
    XCTAssert(![provider isAddress:primeriAddress containsTx:[BTTxTestData getTx:txCount - 1]]);
    ...
    XCTAssertEqual(txCount, [provider getTxAndDetailByAddress:primeriAddress].count);
    ...
    for (int i = 0; i < txCount; i++) {
        ...
        if (i != txCount - 1) {
            for (BTOut *outItem in item.outs) {
                if (outItem.outAddress == primeriAddress) {
    ...
    NSArray *txs = [provider getTxAndDetailByAddress:primeriAddress];
    for (int i = 0; i < txCount; i++) {
...
- (void)testConfirmTx;{
    ...
    XCTAssertEqual(0, [provider getTxAndDetailByAddress:primeriAddress].count);
    [provider add:unconfirmTx[0]];
    XCTAssertEqual(1, [provider getTxAndDetailByAddress:primeriAddress].count);
